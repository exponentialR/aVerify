...
def create_or_select_project():
    projects = [{'name': d, 'created_at': datetime.now().strftime('%d %b \'%y, %H:%M')} for d in os.listdir(app.config['PROJECTS_FOLDER']) if os.path.isdir(os.path.join(app.config['PROJECTS_FOLDER'], d))]
